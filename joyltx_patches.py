...
try:
    ...
    if not getattr(_lu.BlurDownsample, "_joyltx_device_safe", False):
        _lu.BlurDownsample.forward = _blur_forward
        _lu.BlurDownsample._joyltx_device_safe = True
        logging.info("[JoyLTX] LTX rational upsampler: blur kernel follows "
                     "the input device (partial-load safe)")
except Exception as _e:                                   # pragma: no cover
    logging.info("[JoyLTX] LTX upsampler patch not installed (%s)", _e)
