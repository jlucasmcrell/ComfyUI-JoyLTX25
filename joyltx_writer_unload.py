...
def _resident(root, timeout=10.0):
    ...
    try:
        ...
        print("[JoyLTX] writer unload: could not query %s/api/ps (%s) - "
...
def _unload(base_url, model_name, timeout=10.0):
    ...
    if before is None:
        print("[JoyLTX] writer unload: %s has no native API (hosted endpoint?) "
              "- nothing on this card to free." % root, flush=True)
        return
    if before == "unknown":
        before = []                      # proceed to the unload blind
    elif not before:
        print("[JoyLTX] writer unload: nothing resident.", flush=True)
        return
    ...
    for name in targets:
        try:
            ...
            print("[JoyLTX] writer unload: %s did not unload (%s) - continuing."
                  % (name, e), flush=True)
    ...
    print("[JoyLTX] writer unload: freed %.1f GB from %s%s"
          % (freed, root, ("; still resident: " + still) if still
             else "; card is clear"), flush=True)


def _patch(cls, name):
    if name in _PATCHED or getattr(cls, "_rift_unload_patched", False) or getattr(cls, "_joyltx_unload_patched", False):
        return False
        ...
        try:
            ...
            if do:
                try:
                    ...
                    print("[JoyLTX] writer unload skipped: %s" % e, flush=True)

    cls.INPUT_TYPES = INPUT_TYPES
    setattr(cls, fn_name, wrapped)
    cls._joyltx_unload_patched = True
    cls._rift_unload_patched = True   # the H3 pack checks this flag: never patch twice
    _PATCHED.add(name)
    print("[JoyLTX] %s: added '%s' switch (frees the writer's VRAM before the "
        ...
        for t in missing:
            ...
            if cls is not None:
                try:
                    ...
                    logging.warning("[JoyLTX] could not patch %s: %s", t, e)
                    _PATCHED.add(t)
        time.sleep(1.0)
